...
const createAuthorService = async (data) => {
    ...
    const existingUser = await userModel.findOneAndUpdate({ email }, { role: 'author' })
    ...
    await sendEmail(EMAIL_USER, email, 'Request permission to become author', `You are now author`)
}
