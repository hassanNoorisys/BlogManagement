...
import { addToFavourite, createBlog, deleteBlog, favouriteBlog, getBlogOnState, getBlogs, likeOrDislike, setBlogState, updateBlog } from '../controllers/blog.controller.js';
...
route.get('/', getBlogs)
    ...
    .get('/state', verifyToken, verifyRole('Admin', 'Author'), getBlogOnState)


route.patch('/:id', blogImageUpload.array('images'), verifyToken, verifyRole('Admin', 'Author'), updateBlog)
    .patch('/state/:id', verifyToken, verifyRole('Admin', 'Author'), setBlogState)  // change blog state to active or inactive
